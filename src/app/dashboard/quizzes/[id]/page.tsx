...
export default function TakeQuizPage({ params }: { params: { id: string } }) {
  const { data, error, isLoading } = useSWR(`/api/quizzes/${params.id}`, fetcher);
  ...
  if (error || !data || !data.quiz) notFound();
  ...
  if (isFinished) {
    return <QuizResults quiz={quiz} score={score} selectedAnswers={selectedAnswers} />;
  }
...
function QuizResults({ quiz, score, selectedAnswers }: { quiz: IQuiz, score: number, selectedAnswers: (number | null)[] }) {
